import * as assert from "uvu/assert";
import * as marshal from "../src";
import { describe, rubyMarshalDump } from "./helper";

describe("parse", test => {
  test("trivial value", async () => {
    assert.is(marshal.load(await rubyMarshalDump(`nil`)), null);
    assert.is(marshal.load(await rubyMarshalDump(`true`)), true);
    assert.is(marshal.load(await rubyMarshalDump(`false`)), false);
    assert.is(marshal.load(await rubyMarshalDump(`0`)), 0);
    assert.equal(marshal.load(await rubyMarshalDump(`[]`)), []);
    assert.instance(marshal.load(await rubyMarshalDump(`{}`)), marshal.RubyHash);
  });

  test("number", async () => {
    assert.is(marshal.load(await rubyMarshalDump(`114514`)), 114514);
    assert.is(marshal.load(await rubyMarshalDump(`-1919810`)), -1919810);
    assert.is(marshal.load(await rubyMarshalDump(`1145141919810`)), 1145141919810);
    assert.is(marshal.load(await rubyMarshalDump(`123.456`)), 123.456); // please, don't compare float
  });

  test("string", async () => {
    const repeat = (Math.random() * 100) | 0;
    assert.is(marshal.load(await rubyMarshalDump(`'a' * ${repeat}`)), "a".repeat(repeat));
  });

  test("extended", async () => {
    let obj: marshal.RubyObject = marshal.load(
      await rubyMarshalDump(`module M end; class A end; a = A.new; a.extend M; a`)
    );
    assert.is(obj.className, Symbol.for("A"));
    assert.equal(obj.extends, [Symbol.for("M")]);

    let obj2: marshal.RubyObject = marshal.load(
      await rubyMarshalDump(`module M end; class A end; a = A.new; a.singleton_class.prepend M; a`)
    );
    assert.is(obj2.className, Symbol.for("A"));
    assert.equal(obj2.extends, [Symbol.for("M"), Symbol.for("A")]);
  });

  test("circular", async () => {
    let arr: any[] = marshal.load(await rubyMarshalDump(`a = []; a << a; a`));
    assert.is(arr[0], arr);

    let hash: marshal.RubyHash = marshal.load(await rubyMarshalDump(`a = {}; a[:a] = a; a`));
    assert.is(hash.entries[0][1], hash);

    let obj: marshal.RubyObject = marshal.load(
      await rubyMarshalDump(`a = Object.new; a.instance_variable_set :@a, a; a`)
    );
    assert.is(obj.instanceVariables?.[0][1], obj);
  });
});
